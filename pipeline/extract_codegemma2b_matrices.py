#!/usr/bin/env python3
"""
Enhanced CodeGemma-2B LoRA A/B Matrix Extractor

This script extracts LoRA A and B matrices from CodeGemma-2B checkpoint files and also computes
their products (A*B) to get the LoRA change matrices. It processes all four
CodeGemma-2B checkpoints: annotated, concatenationTrained, multiline, and singleline.
...
def simplify_layer_key(full_key: str) -> str:
    ...
    Example:
        base_model.model.model.layers.0.self_attn.q_proj -> layer_00_self_attn_q_proj
        base_model.model.model.layers.15.self_attn.k_proj -> layer_15_self_attn_k_proj
    """
    # Remove base_model.model.model. prefix
    if full_key.startswith('base_model.model.model.'):
        key = full_key[len('base_model.model.model.'):]
    else:
        key = full_key
    
    # Parse layers.X.component format
    parts = key.split('.')
    if len(parts) >= 3 and parts[0] == 'layers':
        layer_num = int(parts[1])
        component_parts = parts[2:]  # self_attn.q_proj, etc.
...
def extract_all_codegemma2b_matrices(config_path: str, output_dir: str) -> None:
    """
    Main function to extract LoRA A, B, and AB matrices from all CodeGemma-2B checkpoints.
    """
    print("🚀 ENHANCED CODEGEMMA-2B LORA MATRIX EXTRACTION")
    print("=" * 60)
    print("Features:")
    ...
    metadata = {
        'extraction_type': 'enhanced_codegemma2b_lora_matrices',
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced LoRA matrix extraction from CodeGemma-2B checkpoints",
        epilog="Extracts A, B, and AB matrices in multiple formats for different optimization approaches"
    )
    parser.add_argument(
        "--config", 
        default="checkpoints_config_gemma.yml",
        help="Path to checkpoints configuration YAML file"
    )
    parser.add_argument(
        "--output", 
        default="enhanced_extracted_codegemma2b_matrices",
    ...
    print(f"Enhanced LoRA Matrix Extraction")
    ...
    # Run extraction
    try:
        extract_all_codegemma2b_matrices(config_path, output_dir)
